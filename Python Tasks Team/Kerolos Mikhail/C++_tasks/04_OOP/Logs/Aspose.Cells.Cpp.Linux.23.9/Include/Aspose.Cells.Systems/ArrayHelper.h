	...
	namespace Cells {
		namespace Systems {
			...
			public:
				...
				static void Copy(T** sourceArray, int sourceArrayIndex, U** destinationArray, int destinationArrayIndex, int length) {
					if (sourceArray == destinationArray) {
						T** src = sourceArray + sourceArrayIndex;
						U** dst = destinationArray + destinationArrayIndex;
						for (int i = 0; i < length; i++, src++) {
							if (*src != NULL) {
								(*src)->add_ref();
							}
				...
				*/
				template <class T>
				static void CopyDiffPtr(T** sourceArray, int sourceArrayIndex, Object** destinationArray, int destinationArrayIndex, int length) {
					if ((void*)sourceArray == (void*)destinationArray) {
						T** src = sourceArray + sourceArrayIndex;
						Object** dst = destinationArray + destinationArrayIndex;
						for (int i = 0; i < length; i++, src++) {
							if (*src != NULL) {
								(*src)->add_ref();
							}
				...
				///static int BinarySearch(intrusive_ptr<Array> array, int index, int length, ObjectPtr value);//error!!!
				///static int BinarySearch(intrusive_ptr<Array> array, ObjectPtr value); //error!!!

				template <class T>
				static int BinarySearch(intrusive_ptr<Array1D<T*>> array, int index, int length, ObjectPtr value)
				{
					if (array == NULL) {
						throw ArgumentNullException("array", "NULL");
					}

					int lowerBound = 0;
					if (index < lowerBound || length < 0) {
						throw ArgumentOutOfRangeException((index < lowerBound) ? "index" : "length", "ArgumentOutOfRange_NeedNonNegNum");
					}
					int arrayLength = array->Length();
					if (arrayLength - (index - lowerBound) < length) {
						throw ArgumentException("Argument_InvalidOffLen");
					}
					if (array->GetRank() != 1)
					{
						throw NEW ArgumentOutOfRangeException("array rank > 1");
					}

					Array1D<T*>* array2 = (Array1D<T*>*)array.get();
					for (int i = 0; i < length; i++)
					{
						int num2 = value->CompareTo(array2->At(index + i));
						if (num2 == 0)
						{
							return index + i;
						}
				...
				template <class T>
				static int BinarySearch(intrusive_ptr<Array1D<T*>> array, ObjectPtr value)
				{
					if (array == NULL) {
						throw ArgumentNullException("array", "NULL");
					}
					...
					{
						int num2 = value->CompareTo(array2->At(i));
						if (num2 == 0)
						{
							return  i;
						}
